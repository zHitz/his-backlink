    ...
    if is_domain_file_valid(os.path.join(base_directory, 'domain_results.txt')):
        ...
        for keyword in search_keywords:
            with open(file_path, 'a', encoding='utf-8') as file:
                ...
                logging.info(f'{num_results} < {max_results}')
                while num_results < max_results:
                    search_results = driver.find_elements(By.CSS_SELECTOR, 'div.g')
                    print(search_results)
                    logging.info('driver.find_elements(By.CSS_SELECTOR, "div.g")')
                    for result in search_results:
                        try:
                            logging.info(f'Try to find element {result}')
                    ...
                    if num_results >= max_results:
                        logging.info(f'num_results >= max_results: {num_results} >= {max_results}')
                        break

                    current_scroll_y = driver.execute_script('return window.scrollY')
                    if current_scroll_y == previous_scroll_y:
                        no_scroll = True
                        logging.info(f'current_scroll_y == previous_scroll_y :{num_results} == {max_results}')
                        break
