...
temp_file_path = os.path.join(base_directory, f'temp_data.xlsx')
...
logging.info('Gộp dữ liệu ngày vào file chung')   
...
data1_file_path = os.path.join(base_directory, f'results_backlink_{current_datetime}.xlsx')
data1 = pd.read_excel(data1_file_path)

data2_file_path = os.path.join(base_directory, f'temp_data.xlsx')
data2 = pd.read_excel(data2_file_path)
...
now = datetime.now()
logging.info('Kiểm tra ngày làm báo cáo')
...
if now.weekday() == 4 and now.hour >= 7:
    ...
    RAW_DATA_file_path = os.path.join(base_directory, f'RAW_data.xlsx') 
    ...
    df.to_excel(file_excel_report_raw, index=False)

    logging.info("Các giá trị trùng lặp trong cột A đã được xóa và số lượng bị trùng đã được tính. Lưu vào file Excel mới.")
    ...
    shutil.move(file_path, file_excel_report_edited)

    logging.info('Send Excel report')
    # Replace 'YOUR_API_TOKEN' with your actual Telegram Bot API token
    api_token = '5730563947:AAGCUmNjCD3FZdeUXVAmM0xmaMoaJbDEBbk'
    # Use your channel chat ID
    chat_id = '-1001924407467'  
    ...
    if response_doc_edited.status_code == 200:   
        ...
        if os.path.exists(file_excel_report_edited):
            logging.info('Xóa file rp EDITED')
            os.remove(file_excel_report_edited)
        
logging.info('Xóa file hằng ngày')        
